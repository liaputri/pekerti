...
exports.userDetail = function (req, res) {
    var query = connection.query('select * from users where id_user = ?', req.params.id, function (err, userDetail) {
        ...
        var query = connection.query('select * from user_jawaban_java', function (err, soalSummary) {

            console.log(soalSummary);
            let nilai = null;         
            for (let z=0 ; z < soalSummary.length ; z+=1){

                if (soalSummary[z].status === 'Jawaban Anda Benar'){
                    nilai +=10;
                }
            ...
            res.render('admin/detail', {
                dataUser: userDetail[0],
                nilaiPraktikan: nilai
            });
...
exports.videoJava = function (re, res) {
